...
import {
  WebGLErrorBoundary,
  WebGLFallback,
} from "./animated-gradient-utils/webgl-error-boundary";
  ...
  useEffect(() => {
    ...
    void main() {
      ...
      const animate = (time: number) => {
        ...
        gl.uniform1f(
          uniforms.u_time,
          elapsed * speed + params.offset * 0.01
        );
...
vec4 blend_colors(
  ...
  float edge_blur
    ...
    float r1 = smoothstep(
      .0 + .35 * edgesWidth,
      .7 - .35 * edgesWidth + .5 * edge_blur,
      mixer
    );

    float r2 = smoothstep(
      .3 + .35 * edgesWidth,
      1. - .35 * edgesWidth + edge_blur,
      mixer
    );

    vec3 blended_color_2 =
      mix(color1, color2, r1);

    float blended_opacity_2 =
      mix(c1.a, c2.a, r1);

    vec3 c =
      mix(blended_color_2, color3, r2);

    float o =
      mix(blended_opacity_2, c3.a, r2);
...
void main() {
    ...
    if (u_shape < .5) {
      ...
      shape =
        .5 +
        .5 *
        sin(checks_shape_uv.x) *
        cos(checks_shape_uv.y);
